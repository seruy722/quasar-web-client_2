import { formatISO, isDate, format, set } from "date-fns";
import { isString, isFinite, isNaN, split, join, reverse, trim } from "lodash";
/**
...
export const checkDate = (input) =>
  isString(input) && !!new Date(input).getTime();
...
export const formatTimer = (date) => {
  date = isFinite(date) ? date : Date.parse(date);
  let result = false;
  if (!isNaN(date)) {
...
export const toDate = (str) => {
  if (str && size(str) === 19) {
    const [date, time] = split(str, " ");
    const parseDate = join(reverse(split(date, "-")), "-");
    return `${parseDate} ${time}`;
  }
  if (trim(str) && isDate(new Date(str))) {
    toDate(fullDate(str));
  }
...
export const getTimeZone = () => {
  ...
  return timeZoneOffset < 0
    ? `+0${(timeZoneOffset / 60) * -1}:00`
    : `-0${timeZoneOffset / 60}:00`;
};
