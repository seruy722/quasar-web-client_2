import { findIndex, isArray, forEach } from "lodash";
...
export const UPDATE_CARGO_ENTRY = (state, data) => {
  if (isArray(data)) {
    forEach(data, (item) => {
      const index = findIndex(state.cargo, { id: item.id });
    ...
    const index = findIndex(state.cargo, { id: data.id });
...
export const DELETE_CARGO_ENTRY = (state, data) => {
  forEach(data, (id) => {
    const index = findIndex(state.cargo, { id });
    if (index !== -1) {
      state.cargo.splice(index, 1);
    }
  });
...
export const UPDATE_DEBT_ENTRY = (state, data) => {
  if (isArray(data)) {
    forEach(data, (item) => {
      const index = findIndex(state.debts, { id: item.id });
      if (index !== -1 && state.currentCodeClientId === item.code_client_id) {
    ...
    const index = findIndex(state.debts, { id: data.id });
    if (index !== -1 && state.currentCodeClientId === data.code_client_id) {
...
export const DELETE_DEBT_ENTRY = (state, data) => {
  forEach(data, (id) => {
    const index = findIndex(state.debts, { id });
    if (index !== -1) {
      state.debts.splice(index, 1);
    }
  });
