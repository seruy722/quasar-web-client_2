<template>
  <div data-vue-component-name="TransfersStatistics">
    ...
    <q-list v-if="!isEmptyData" bordered separator>
      ...
      <q-item v-for="({ count, sum }, index) in statistics" :key="index">
        <q-item-section>
          <q-item-label>
    ...
    <q-list v-if="!isEmptyData" bordered separator>
      ...
      <q-item v-for="(user, index) in usersStatistics" :key="index">
        <q-item-section>
        ...
        <q-card-section class="q-pt-none">
          <div v-show="statisticsSelectData.value === 2">
            <div class="row items-center">
              <span>От:</span>
              <DateWithInput v-model:value="period.from" />
            </div>
            <div class="row items-center">
              <span>До:</span>
              <DateWithInput v-model:value="period.to" />
            </div>
          </div>
          <div v-show="statisticsSelectData.value === 3">
            <DateWithInput v-model:value="period.day" />
          </div>
        </q-card-section>
...
export default defineComponent({
  ...
  computed: {
    isEmptyData() {
      return isEmpty(this.statistics);
    },
  },
  created() {
  ...
  methods: {
    summa(obj, field) {
      if (!isArray(obj)) {
        return numberFormat(reduce(obj, (sum, item) => sum + item[field], 0));
      }
      return null;
    },
    numberFormat,
    ...
    getStatistics(selectValue, values) {
      return this.$axios.post(getUrl("transfersStatistics"), {
        selectValue,
        values,
      });
    },
    getUsersDataForTransfers(statistics) {
      const obj = {};
      forEach(statistics, (elem) => {
        const usersIds = uniq(map(elem.countUserTransfers, "user_id"));
        forEach(usersIds, (id) => {
          const userData = filter(
            elem.countUserTransfers,
            (item) => item.user_id === id,
          );
          if (!obj[id]) {
            obj[id] = {
              name: get(first(userData), "user_name"),
              sum: sumBy(userData, "sum"),
              count: size(userData),
            };
          } else {
            obj[id]["sum"] += sumBy(userData, "sum");
            obj[id]["count"] += size(userData);
          }
        });
      });
      const arr = filter(obj, (item) => item);
      return orderBy(arr, "sum", "desc");
    },
    async setStatisticsData(data, date) {
      ...
      if (this.statisticsSelectData.value === -1) {
        this.viewPeriodDate = "";
        this.getStatistics(-1, null).then(({ data: { statistics } }) => {
        ...
        this.getStatistics(3, { day }).then(({ data: { statistics } }) => {
          this.statistics = statistics;
          this.usersStatistics = this.getUsersDataForTransfers(statistics);
        });
        ...
        this.getStatistics(2, { from, to }).then(({ data: { statistics } }) => {
          this.statistics = statistics;
          this.usersStatistics = this.getUsersDataForTransfers(statistics);
        });
      } else {
        this.viewPeriodDate = date;
        this.getStatistics(0, { day: date }).then(
          ({ data: { statistics } }) => {
            this.statistics = statistics;
            this.usersStatistics = this.getUsersDataForTransfers(statistics);
          },
