<template>
  <div data-vue-component-name="TransfersListSkeleton">
    <slot>
      <div v-if="$q.screen.xs" class="q-mt-md">
        <ListItem
          v-for="(n, i) in 8"
          :key="i"
          dense
          class="shadow-1 overflow-hidden q-ma-sm"
          style="border-radius: 30px; border: 1px solid lightgray"
        >
          <ItemSection avatar>
            <Skeleton type="QCheckbox" />
          </ItemSection>
        ...
        <q-markup-table>
          ...
          <tbody>
            <TR v-for="n in 5" :key="n">
              ...
              <td v-for="n in 5" :key="n" class="text-right">
                <Skeleton
                  animation="blink"
                  type="text"
                  :width="`${Math.floor(Math.random() * (85 - 35 + 1)) + 35}px`"
                />
              </td>
            </TR>
          </tbody>
