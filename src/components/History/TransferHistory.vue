<template>
  <RoundBtn icon="history" tooltip="История" @round-btn-click="show = true" />
  <DialogComponent
    :dialog="show"
    :maximized="true"
  ...
  >
    <q-card style="max-width: 600px" data-vue-component-name="TransferHistory">
        ...
        <q-timeline color="secondary">
          ...
          >
            <List separator dense>
              <div v-for="(history, i) in transfer" :key="i">
                <ListItem
                ...
                >
                  ...
                  <ItemSection side>
                    <ItemLabel v-if="i === 'notation'" :lines="3">
                      {{ history }}
                    </ItemLabel>
                    <ItemLabel v-else-if="i === 'status'">
                      <Badge :color="statusColor(transfer['status_label'])">
                        {{ transfer["status_label"] }}
                      </Badge>
                    </ItemLabel>
                    <ItemLabel v-else-if="i === 'method'">
                      {{ transfer["method_label"] }}
                    </ItemLabel>
                    <ItemLabel v-else-if="i === 'receiver_phone'">
