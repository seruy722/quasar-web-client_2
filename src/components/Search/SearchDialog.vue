...
import { assign, map, isEmpty, pickBy } from "lodash";
...
export default {
  ...
  components: {
    ...
    ClientsSelectComponent: defineAsyncComponent(
      () => import("src/components/Search/ClientsSelectComponent.vue"),
    ),
    UserSelectComponent: defineAsyncComponent(
      () => import("src/components/Search/UserSelectComponent.vue"),
    ),
    StatusSelectComponent: defineAsyncComponent(
      () => import("src/components/Search/StatusSelectComponent.vue"),
    ),
    DateComponent: defineAsyncComponent(
      () => import("src/components/Search/DateComponent.vue"),
    ),
    DateComponent2: defineAsyncComponent(
      () => import("src/components/Search/DateComponent2.vue"),
    ),
    MethodsSelectComponent: defineAsyncComponent(
      () => import("src/components/Search/MethodsSelectComponent.vue"),
    ),
    SumComponent: defineAsyncComponent(
      () => import("src/components/Search/SumComponent.vue"),
    ),
    PaidFieldComponent: defineAsyncComponent(
      () => import("src/components/Search/PaidFieldComponent.vue"),
    ),
    NotationFieldComponent: defineAsyncComponent(
      () => import("src/components/Search/NotationFieldComponent.vue"),
    ),
    AllFieldsComponent: defineAsyncComponent(
      () => import("src/components/Search/AllFieldsComponent.vue"),
    ),
    ReceiverNameFieldComponent: defineAsyncComponent(
      () => import("src/components/Search/ReceiverNameFieldComponent.vue"),
    ),
    ReceiverPhoneFieldComponent: defineAsyncComponent(
      () => import("src/components/Search/ReceiverPhoneFieldComponent.vue"),
    ),
  },
  props: {
    loading: {
  ...
  setup(props, { emit }) {
    ...
    const optionsFields = map(props.columns, ({ label, name }) =>
      assign(
        {},
        {
          label,
    ...
    const filterMethod = (val) => {
      if (val.paid === 0 || !isEmpty(pickBy(val, (v) => !!v))) {
    ...
    const clearSearchData = () => {
      ...
      emit("update:loading", true);
      showDialog.value = false;
      transfersStore.fetchTransfers().finally(() => {
        emit("update:loading", false);
      });
    };
