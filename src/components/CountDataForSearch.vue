<template>
  <div v-show="list.length" data-vue-component-name="CountDataForSearch">
...
export default {
  ...
  methods: {
    ...
    async setLocalList(list, transporterPriceData) {
      const { setCargoCategoriesData } = await import(
        "src/utils/FrequentlyCalledFunctions"
      );
      const { categoriesList, footer } = setCargoCategoriesData(
        list,
