      ...
      <template #default="{ item }">
        <q-list separator bordered class="rounded-borders">
          ...
          >
                  ...
                  <q-icon
                    ...
                    @click.stop="
                      $router.push({
                        ...
                        params: {
                          id: item.fax_id,
                          searchField: 'code_client_name',
                          searchValue: item.code_client_name,
                        },
                      })
                    "
                  />
            ...
            <q-list v-if="!$q.screen.xs" bordered separator dense>
              ...
              <q-item v-for="(elem, index) in item.children" :key="index">
                <q-item-section
            ...
            <div v-else>
              ...
              >
                <q-item v-for="(el, ind) in headerColumns" :key="ind">
                  <q-item-section>
...
import { isEmpty, find as findLodash, get } from "lodash";
  ...
  methods: {
    ...
    beforeShowExpansionPanel(item, childrenValues) {
      if (isEmpty(item.children)) {
        const find = findLodash(childrenValues, { faxId: item.fax_id });
        if (find) {
