...
export default defineComponent({
  ...
  components: {
    ...
    TransferHistory: defineAsyncComponent(
      () => import("src/components/History/TransferHistory.vue"),
    ),
  },
  props: {
    showDialog: {
  ...
  setup(props, { emit }) {
    ...
    const dialogTitle = computed(
      () => get(props.localProps, "row.client_name") || "Новый перевод",
    );
    const show = computed({
      get: function get() {
    ...
    watch(newCodeData, (val) => {
      if (!isEmpty(val)) {
        set(props.transferData, "client_id.value", val.value);
        set(props.transferData, "client_id.changeValue", true);
      }
    });

    function changeValueFunc(id) {
      if (isEmpty(props.localProps)) {
        axiosInstance
          .get(`${getUrl("getTransferCodeCommission")}/${id}`)
          .then(({ data: { transfer } }) => {
            if (transfer) {
              set(
                props.transferData,
                "transfer_commission.value",
                transfer.transfer_commission || 1,
              );
            }
          });
      }
    ...
    function updateData(data) {
      ...
      if (!sendData.client_id.value) {
      ...
      } else if (isEmpty(props.localProps)) {
        // ДОБАВЛЕНИЕ ЗАПИСИ
        $q.loading.show();
        ...
        axiosInstance
          .post(getUrl("storeTransfers"), values)
          .then(({ data: { transfer } }) => {
          ...
          .catch((errors) => {
            $q.loading.hide();
            errorsData.errors = get(errors, "response.data.errors");
          });
      } else if (has(props.localProps, "row.id")) {
        // ОБНОВЛЕНИЕ ЗАПИСИ
        if (some(sendData, "changeValue")) {
          ...
          assign(dataToSend, { id: get(props.localProps, "row.id") });
          axiosInstance
            .post(getUrl("updateTransfers"), dataToSend)
            .then(
              ({
                data: {
                  transfer: { client_name: clientName },
                },
              }) => {
                cancel(props.transferData);
                $q.loading.hide();
                ...
                showNotif(
                  "success",
                  `Запись клиента - ${clientName} успешно обновлена.`,
                );
              },
            )
            .catch((errors) => {
