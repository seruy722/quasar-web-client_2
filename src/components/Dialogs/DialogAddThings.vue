<template>
  ...
  >
    <Card style="min-width: 320px; width: 100%; max-width: 900px">
      ...
      <CardSection>
        <div
          class="fit row wrap justify-start items-center content-start bg-white"
        >
          <div
        ...
        <List dense bordered padding separator>
          <ListItem>
...
import { isEmpty, forEach, isString, startCase } from "lodash";
import { useThingsListStore } from "stores/things-list";
const thingsListStore = useThingsListStore();
...
export default {
  ...
  computed: {
    thingsList() {
      return thingsListStore.getThingsList;
    },
    size() {
  ...
  methods: {
    ...
    add({ count: { value: countValue }, title: { value: titleValue } }) {
      this.localThings.unshift({
        count: countValue,
        title: startCase(titleValue),
      });
    },
    save(val) {
