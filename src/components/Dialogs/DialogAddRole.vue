    ...
    >
      <Card style="min-width: 320px; width: 100%; max-width: 500px">
        <CardSection class="row justify-between bg-grey q-mb-sm">
          <span class="text-h6">{{
            roleName ? "Редактирование роли" : "Новая роль"
          }}</span>
          <div>
            <IconBtn
              dense
      ...
      if (this.roleName) {
        this.$axios
          ...
          .then(({ data: { users } }) => {
            ...
            this.showNotif(
              "success",
              `Роль ${value} успешно задана пользователям ${usersValue.join(
                ",",
              )}.`,
              "center",
            );
        ...
        this.$axios
          ...
          .then(({ data: { addedRole, users } }) => {
            rolesStore.addRole(addedRole);
            authStore.setUsersWithRolesAndPermissions(users);
            this.showNotif(
              "success",
              `Роль ${addedRole.name} успешно добавлена.`,
              "center",
            );
            this.close(this.roleData);
            this.$q.loading.hide();
