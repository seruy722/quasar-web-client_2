<template>
  <DialogComponent v-model:dialog="show" title="Код" :persistent="true">
    <Card
  ...
  <DialogAddClient v-model:show-dialog="showClientDialog" :code-id="codeId" />
</template>
...
import { isEmpty, startCase, first } from "lodash";
...
export default defineComponent({
  ...
  components: {
    ...
    BaseBtn,
    BaseInput,
    DialogAddClient: defineAsyncComponent(
      () => import("src/components/Dialogs/DialogAddClient.vue"),
    ),
    Separator,
    Card,
    CardActions,
  ...
  setup(props, { emit }) {
    ...
    function saveData({ code: { value } }) {
      loading.value = true;
      axiosInstance
        .post(getUrl("storeCode"), { code: startCase(value) })
        .then(({ data: { code: addedCode, codeWithCustomers } }) => {
          if (!isEmpty(codesStore.getCodes)) {
            codesStore.addCode({
              code: addedCode,
              codeWithCustomers: first(
                setFormatedDate(codeWithCustomers, ["created_at"]),
              ),
            });
          } else {
            codesStore.setCodes();
