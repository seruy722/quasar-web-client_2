  ...
  methods: {
    saveData({
      ...
      if (this.permissionName) {
        this.$axios
          .post(getUrl("assignPermissionToRoleAndUser"), {
            name: value,
            users: usersNames,
            roles: rolesNames,
          })
          .then(({ data: { users } }) => {
            ...
            this.showNotif(
              "success",
              `Разрешение ${value} успешно добавлено пользователям ${usersNames.join(
                ",",
              )} и ролям ${rolesNames.join(",")}.`,
              "center",
            );
        ...
        this.$axios
          .post(getUrl("addPermission"), {
            name: value,
            users: usersNames,
            roles: rolesNames,
          })
          .then(({ data: { addedPermission, users } }) => {
            permissionsStore.addPermission(addedPermission);
            authStore.setUsersWithRolesAndPermissions(users);
            this.showNotif(
              "success",
              `Разрешение ${addedPermission.name} успешно добавлено.`,
              "center",
            );
            this.close(this.permissionData);
            this.$q.loading.hide();
