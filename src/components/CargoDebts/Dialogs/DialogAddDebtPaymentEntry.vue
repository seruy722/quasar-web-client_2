  ...
  methods: {
    async saveData() {
      ...
      if (!this.entryData.row) {
        ...
        this.$q.loading.show();
        this.$axios
          .post(getUrl("createDebtPaymentEntry"), sendData)
          .then(({ data: { answer } }) => {
            cargoDebtsStore.addDebtEntry(answer);
            this.$q.loading.hide();
        ...
        this.$q.loading.show();
        this.$axios
          .post(getUrl("updateDebtPaymentEntry"), sendData)
          .then(({ data: { answer } }) => {
            cargoDebtsStore.updateDebtEntry(answer);
