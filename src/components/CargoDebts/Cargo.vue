<template>
  <div data-vue-component-name="CargoComponent">
    <TableComponent
    ...
    >
      <template #top-buttons>
        <div class="row q-gutter-sm">
          <MenuCargo v-show="currentCodeClientId" />
          ...
          <RoundBtn
            ...
            tooltip="Удалить"
            @round-btn-click="
              destroyEntry(cargoTableReactiveProperties.selected)
            "
          />
          ...
          <q-btn-dropdown split color="positive" icon="explicit" dense>
            <q-list separator>
              ...
              <q-item
                ...
                clickable
                @click="
                  exportDetailFaxData(cargoTableReactiveProperties.selected)
                "
              >
                <q-item-section>
              ...
              <q-item v-close-popup clickable @click="exportZeroCargoData">
                <q-item-section>
          ...
          <RoundBtn
            ...
            @round-btn-click="
              calculateClient(cargoTableReactiveProperties.selected, cargo)
            "
          />
        </div>
      </template>
      ...
      <template #inner-body="{ props }">
        ...
        >
          ...
          <q-td key="for_kg" class="text-bold cursor-pointer" :props="props">
            {{ props.row.type ? null : props.row.for_kg }}
          </q-td>

          <q-td key="for_place" class="text-bold cursor-pointer" :props="props">
          ...
          <q-td key="fax_name" :props="props" class="faxName">
            <router-link
              :to="{ name: 'fax', params: { id: props.row.fax_id } }"
            >
              {{ props.row.fax_name }}
            </router-link>
          </q-td>
          ...
          <q-td key="in_cargo" :props="props">
            <q-badge :color="props.row.in_cargo ? 'positive' : 'negative'">
              {{ props.row.in_cargo ? "Да" : props.row.type ? null : "Нет" }}
            </q-badge>
          </q-td>
          ...
          <q-td key="delivery_method_name" :props="props">
...
export default {
  ...
  computed: {
    ...
    titleTable() {
      return `Сумма: ${round(sumBy(this.cargo, "sum"))}, Скидки: ${round(
        sumBy(this.cargo, "sale"),
      )}`;
    },
  },
  methods: {
    ...
    async viewEditDialog(data, event) {
      if (!includes(get(event, "target.classList"), "select_checkbox")) {
        if (data.row.type) {
          ...
          !data.row.type &&
          !includes(event.target.classList, "faxName") &&
          has(data, "row.arr")
        ) {
          const { setFormatedDate } = await import(
            "src/utils/FrequentlyCalledFunctions"
          );
          this.entryData = setFormatedDate(cloneDeep(get(data, "row.arr")), [
            "created_at",
          ]);
          this.showDialogViewCargoData = true;
        } else {
          this.entryData = [get(data, "row")];
          this.showDialogViewCargoData = true;
        }
      }
    },
          ...
          [
            ...
            },
            {
              ...
              handler: async () => {
                ...
                this.$q.loading.show();
                this.$axios
                  .post(getUrl("deleteCargoEntry"), { ids })
                  .then(() => {
                    cargoDebtsStore.deleteCargoEntry(ids);
                    this.cargoTableReactiveProperties.selected = [];
                    this.$q.loading.hide();
                    this.showNotif(
                      "success",
    ...
    async exportFaxData(selected) {
      const data = !isEmpty(selected) ? selected : this.cargo;
      if (!isEmpty(data)) {
        const clientName = `${get(first(data), "code_client_name")} код карго`;
        const { getUrl } = await import("src/tools/url");
        this.exportDataToExcel(
          getUrl("exportCargoData"),
          {
            data,
          },
          `${clientName}.xlsx`,
    ...
    async exportZeroCargoData() {
      ...
      const { getUrl } = await import("src/tools/url");
      const reversedItems = this.cargo.toReversed();
      const options = {
        year: "numeric",
        month: "numeric",
        day: "numeric",
          ...
          [
            ...
            },
            {
              ...
              handler: async () => {
                ...
                this.$q.loading.show();
                this.$axios
                  .get(`${getUrl("cargoPaymentsAll")}/${id}`)
                  .then(({ data: { cargo } }) => {
                    cargoDebtsStore.updateCargoEntry(cargo);
                    this.$q.loading.hide();
                    this.showNotif(
                      "success",
    ...
    calculateClient(selected, data) {
      if (isEmpty(selected) && isEmpty(data)) {
        this.showNotif("warning", "В таблице нет записей", "center");
      } else if (!isEmpty(selected)) {
        this.calculateData = cloneDeep(selected);
      } else if (!isEmpty(data)) {
        this.calculateData = cloneDeep(data);
      } else {
