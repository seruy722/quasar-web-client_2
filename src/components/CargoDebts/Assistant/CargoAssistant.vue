<template>
  <div data-vue-component-name="Cargo">
    <TableComponent
    ...
    >
      <template #top-buttons>
        <UpdateBtn v-show="currentCodeClientId" @update-btn-click="refresh" />
        <ExportBtn
          @export-btn-click="
            exportFaxData(cargoTableReactiveProperties.selected)
          "
        />
      </template>
            ...
            <template #header>
              ...
              <q-item-section>
                <q-item-label :lines="2">
                  {{
                    props.row.kg
                      ? `${props.row.place}м/${props.row.kg}кг ${props.row.fax_name}`
                      : props.row.type
                        ? props.row.sum
                        : props.row.notation
                  }}
...
import { isEmpty, forEach, orderBy, find } from "lodash";
  ...
  methods: {
    ...
    async exportFaxData(selected) {
      ...
      if (isEmpty(data)) {
        ...
        const searchData = cargoDebtsStore.getCargoForSearch;
        const newArr = [];
        forEach(data, ({ id }) => {
