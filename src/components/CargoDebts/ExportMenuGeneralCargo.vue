<template>
  <div data-vue-component-name="ExportMenuGeneralCargo">
    <RoundBtn
      color="positive"
      tooltip="Excel"
      icon="explicit"
    ...
    <DialogComponent v-model:dialog="show" :persistent="true" title="Оплата">
      <q-card style="min-width: 320px; width: 100%; max-width: 900px">
        <q-card-section
          class="row justify-between items-center bg-grey q-mb-sm"
        >
        ...
        <q-card-section>
          ...
          >
            ...
            />
            <q-btn color="primary" :label="selectData.label">
              <q-menu
                auto-close
...
import { useSitiesStore } from "src/stores/cities";
...
export default {
  ...
  methods: {
    ...
    setDate(val) {
      ...
      if (!val && this.selectData.value === 2) {
        ...
        const to = this.period.to
          ? format(new Date(this.period.to), "dd-MM-yyyy")
          : "";
        const from = this.period.from
          ? format(new Date(this.period.from), "dd-MM-yyyy")
          : "";
    ...
    async exportFaxData(select, type, period, city) {
      ...
      if (select.value === 1) {
        sendData.day = addTime(period.today).toISOString();
      } else if (select.value === 2) {
        sendData.period = period;
        if (period.to) {
          sendData.period.to = addTime(period.to).toISOString();
        }
        if (period.from) {
          sendData.period.from = addTime(period.from).toISOString();
        }
      } else if (select.value === 3) {
        sendData.day = addTime(period.day).toISOString();
      }
      const findCity = find(this.cities, { value: city });
      this.exportDataToExcel(
        getUrl(this.urlName),
        {
          data: sendData,
          cityId: get(findCity, "value"),
        },
        `${this.model}_${findCity ? get(findCity, "label") : "все"}.xlsx`,
      );
      this.show = false;
    },
    async exportGeneralDataByClients(model) {
      this.exportDataToExcel(
        getUrl(this.urlNameClients),
        {
          model,
        },
        `${model}.xlsx`,
    ...
    async exportGeneralDataByClientsCity(model, cityId) {
      const findCity = find(this.cities, { value: cityId });
      this.exportDataToExcel(
        getUrl("exportClientsGeneralDataOdessa"),
        {
          model,
          cityId,
          cityName: get(findCity, "label"),
        },
        `${model}_${findCity ? get(findCity, "label") : "все"}.xlsx`,
      );
    },
  },
};
