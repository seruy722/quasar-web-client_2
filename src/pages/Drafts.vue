<template>
  <div data-vue-component-name="DraftsComponent" class="q-pa-md">
    <div>
    ...
    <div style="border: 1px solid blue">
      ...
      <q-btn label="GET" @click="exportCustomersWhoLeftBrand" />
    </div>
    <div style="border: 1px solid blue">
    ...
    <div style="border: 1px solid blue">
      ...
      <div class="q-pa-md">
        <q-date v-model="date" default-view="Months" mask="MM-YYYY" />
      </div>
    ...
    <div>
      <q-btn label="clearCache" @click="reg" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    upFiles({ target }) {
      ...
      this.$axios
        .post("/api/upload-faxes", formData)
        .then(() => {
          target.value = "";
    ...
    upFaxDataFiles({ target }) {
      const formData = new FormData();
      formData.append("upload", first(target.files));
      formData.append("fax_id", 1);

      this.$axios
        .post("/api/upload-fax-data-table", formData)
        .then(() => {
          target.value = "";
          this.showNotif("success", "Данные успешно добавлены.", "center");
        })
    ...
    upCodes({ target }) {
      ...
      this.$axios
        .post("/api/upload-codes", formData)
        .then(() => {
          target.value = "";
          this.showNotif("success", "Данные успешно добавлены.", "center");
        })
    ...
    searchData() {
      this.$axios
        .post("/api/search-in-faxes", { search: this.search })
        .then(() => {
