<template>
  <q-page data-vue-component-name="SearchPage">
    <q-toggle v-model="visible" label="Показать панель поиска" />
    <q-slide-transition>
      <div v-show="visible">
        <q-card-section>
    ...
    >
      <q-tab name="storehouse" label="СКЛАД">
        <q-badge floating color="red">
          {{ storehouseTableData.data.length }}
        </q-badge>
      </q-tab>
      <q-tab name="faxes" label="ФАКСЫ">
        <q-badge floating color="red">
          {{ faxesTableData.data.length }}
        </q-badge>
      </q-tab>
      <q-tab name="basket" label="Корзина">
        <q-badge floating color="red">
    ...
    <q-tab-panels v-model="tab">
      <q-tab-panel name="storehouse" class="tab-panel">
        <BaseTable
          title="Склад"
      ...
      <q-tab-panel name="faxes" class="tab-panel">
        <keep-alive>
          <ExpansionList
      ...
      <q-tab-panel name="basket" class="tab-panel">
        <BaseTable
          title="Корзина"
  ...
  watch: {
    search(val) {
      if (val) {
        ...
        this.faxesTableData.data = filter(this.faxesTableData.data, (item) => {
          return item.fax_name.toLowerCase().indexOf(valToLower) > -1;
        });
      } else {
        this.faxesTableData.data = clone(this.faxesTableDataDuplicate);
        forEach(this.faxesTableData.data, (item) => {
          if (item.expanded) {
            item.expanded = false;
          }
        });
  ...
  methods: {
    ...
    async fetchClientList() {
      ...
      await this.$axios
        ...
        .catch(() => {
          console.error("Не загрузился список клиентов");
        });
    },
        ...
        this.$axios
          ...
          .then(({ data: { storehouse, faxes, destroyed } }) => {
            ...
            forEach(group, (item) => {
              ...
              const { code_client_name, fax_name, fax_status, fax_id, id } =
                first(item);
              arr.push({
                id,
                fax_id,
            ...
            this.basketTableData.data = map(destroyed, (item) =>
              assign(item, JSON.parse(item.history_data)),
            );
            this.loading = false;
            this.visible = false;
            if (isEmpty(this.storehouseTableData.data)) {
              this.tab = "faxes";
            }
          })
          .catch((errors) => {
