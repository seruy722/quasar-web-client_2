<template>
  <q-card data-vue-component-name="RegisterClient" flat>
    <q-card-section>
      <q-form v-show="!yesPhoneInDB" @submit="getCodeForRegister">
        <q-input
          v-model="phone"
          hint="380501414144"
          type="tel"
          ...
          :rules="[
            (val) =>
              (val && val.length > 0) ||
              'Введите номер телефона - пример:380501414144',
            (val) =>
              (val && val.length === 19) ||
              'Некоректный номер телефона - пример:380501414144',
          ]"
        />
        <div>
          <q-btn label="Получить код" type="submit" color="primary" />
        </div>
      </q-form>
      <q-form
        v-show="yesPhoneInDB"
      ...
      >
        <q-input
          ...
          label="Код из смс"
          :rules="[
            (val) => (val && val.length > 0) || 'Введите код',
            (val) =>
              (val && val.length === 8) || 'Код должен состоять из 6 цифр',
          ]"
        />
        <q-input
          v-model="password"
        ...
        />
        <div>
          <q-btn label="Зарегистрироватся" type="submit" color="primary" />
        </div>
      </q-form>
    </q-card-section>
  </q-card>
...
export default {
  ...
  methods: {
    ...
    getCodeForRegister() {
      ...
      this.$axios
        .post(getUrl("registerClientCode"), sendData)
        .then(({ data: { codeStatus, message } }) => {
          if (codeStatus === 3) {
            this.yesPhoneInDB = true;
            this.showNotif(
              "info",
              `На номер ${this.phone} отправлен код подтверждения`,
              "center",
            );
        ...
        .catch(() => {
          this.showNotif(
            "warning",
            "Слишком много попыток на получение кода. Повторите попытку через 5 минут",
            "center",
          );
        });
    },
    onSubmitRegister() {
      this.$axios
        .post(getUrl("registerClientCodeRegister"), {
