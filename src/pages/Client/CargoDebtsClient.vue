<template>
  <q-page data-vue-component-name="CargoDebtsClient">
    <div class="q-gutter-y-md">
      <q-card>
        <q-tabs
          ...
          align="justify"
        >
          <q-tab name="storehouseData" label="Статус">
            <q-badge color="red">
              {{ storehouseData.length }}
            </q-badge>
          </q-tab>
          <q-tab name="cargo" label="Получено">
            <q-badge color="red">
              {{ cargo.length }}
            </q-badge>
          </q-tab>
          <q-tab name="debts" label="Переводы">
            <q-badge color="red">
              {{ debts.length }}
            </q-badge>
          </q-tab>
        ...
        <q-tab-panels v-model="tab" animated swipeable>
          ...
          <q-tab-panel name="storehouseData" style="padding: 0">
            <Storehouse
...
export default {
  ...
  watch: {
    tab(val) {
      if (val === "storehouseData" && isEmpty(this.storehouseData)) {
        this.$q.loading.show();
  ...
  methods: {
    getClientData() {
      return cargoDebtsStore.getCargoDebts(null);
    },
    async getStorehouseData() {
      const { getUrl } = await import("src/tools/url");
      return this.$axios
        .get(getUrl("getClientStorehouseData"))
        .then(({ data: { clientStorehouseData } }) => {
          this.storehouseData = uniqBy(clientStorehouseData, "code_place");
          this.$q.loading.hide();
        });
    },
    async refresh(done) {
