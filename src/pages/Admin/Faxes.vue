<template>
  <q-page data-vue-component-name="AdminFaxesComponent">
        ...
        <template #top-buttons>
          <div class="row q-gutter-sm">
            ...
            <RoundBtn
              ...
              tooltip="Обьеденить"
              @round-btn-click="
                combineFaxes(faxesTableReactiveProperties.selected)
              "
            />
            <RoundBtn
              icon="flight_land"
...
export default {
  ...
  methods: {
    ...
    setDateFromDialog(date) {
      if (date) {
        this.dialogChooseDataForSend.date = addTime(date).toISOString();
        this.uploadToCargoFinish(this.dialogChooseDataForSend);
      }
    },
    goToFaxData(item) {
      faxesStore.setCurrentFaxItem(find(faxesStore.getFaxes, { id: item.id }));
      this.$router.push({
        name: "admin-fax",
        params: { id: item.id },
      });
    ...
    uploadToCargoFinish({ id, value, date = null }) {
      ...
      this.$axios
        ...
        .catch(() => {
          this.$q.loading.hide();
          console.error("Ошибка загрузки факса в карго");
        });
    },
    uploadToCargo({ id, uploaded_to_cargo: value }) {
      if (!value) {
        this.showNotif(
          "warning",
          value ? "Выгрузить факс из карго?" : "Загрузить факс в карго?",
          "center",
          [
            {
              label: "Отмена",
    ...
    combineFaxes(selectedFaxes) {
      const faxNames = map(selectedFaxes, "name");
      const faxIds = uniq(map(selectedFaxes, "transporter_id"));
      if (!isEmpty(selectedFaxes) && size(faxIds) === 1) {
        this.showNotif(
          "warning",
          `Обьеденить факсы - ${faxNames.join(", ")}?`,
          "center",
          [
            {
              label: "Отмена",
            ...
            },
            {
              ...
              handler: () => {
                this.$q.loading.show();
                this.$axios
                  .post(getUrl("combineFaxes"), selectedFaxes)
                  .then(({ data: { fax } }) => {
                    faxesStore.addFax(
                      setFormatedDate(fax, ["departure_date", "arrival_date"]),
                    );
                    ...
                    this.showNotif(
                      "success",
                      "Факсы успешно обьеденены.",
                      "center",
                    );
                  })
                  .catch((errors) => {
                    console.error("Ошибка запроса combineFaxes", errors);
                    this.showNotif(
                      "error",
                      "Произошла ошибка при обьеденении факсов. Обновите страницу пожалуйста.",
                      "center",
                    );
                    this.faxesTableReactiveProperties.selected = [];
                    this.$q.loading.hide();
        ...
        this.showNotif(
          "warning",
          "Нельзя соеденять факсы разных перевожчиков!",
          "center",
        );
      }
    },
