...
export default {
  ...
  methods: {
    ...
    async getStorehouseDataHistory(id, cols) {
      this.$q.loading.show();
      await this.$axios
        .get(`${getUrl("storehouseDataHistory")}/${id}`)
        .then(({ data: { storehouseDataHistory } }) => {
          if (!isEmpty(storehouseDataHistory)) {
            this.$q.loading.hide();
            this.dialogHistory = true;
            this.storehouseHistoryData = prepareHistoryData(
              cols,
            ...
            this.storehouseHistoryData.historyData = setFormatedDate(
              this.storehouseHistoryData.historyData,
              ["created_at"],
            );
          } else {
            this.$q.loading.hide();
